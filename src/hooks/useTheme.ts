...
import { changeTheme, type ThemeState } from '../store/theme';
import {
  type ThemeVariables,
  type Theme,
...
export default function () {
  ...
  if (currentTheme !== 'default') {
    const {
      Variables,
      // @ts-expect-error to prevent multiple themes handling
      ...themeConfig
    } = themes[currentTheme] || {};

    variables = Variables;
    partialTheme = Boolean(themeConfig) || {};
  }
  ...
  const changeDarkMode = (darkMode: boolean): void => {
